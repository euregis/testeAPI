...
class WorkflowExecutor:
    ...
    def validate(self, validations: List[Dict], response: requests.Response, body_json: Any):
        for v in validations:
            ...
            if not self.compare(actual, operator, expected):
                raise ValueError(f"Validation failed: {target} {operator} {expected}, got {actual}")
    ...
    def run(self, steps_to_run: List[str] = None):
        ...
        for step_name in steps_order:
            ...
            self.validate(step.get("validations", []), response, body_json)
